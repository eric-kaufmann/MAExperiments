...
MODEL_NAME = "single_coord_linear_mlp"
...
def train(args):
    ...
    if GRID64 or GRID128:
        ...
        vessel_dataset = VesselDatasetSinglePoint(DATA_PATH, sample_size=SAMPLE_SIZE, transform_function=transform_function, sample_cap=SAMPLE_CAP)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        model.train()
        epoch_mse_list = []
        for i, (input_tensor, target_tensor, _) in enumerate(train_loader):
        ...
        with torch.no_grad():  # Disable gradient computation
            for i, (input_tensor, target_tensor, _) in enumerate(val_loader):
                input_tensor = input_tensor.to(DEVICE)
                target_tensor = target_tensor.to(DEVICE)
