...
MODEL_NAME = "single_coord_linear_encoder"

# Model Architecture Parameters
ENCODER_SAMPLE_SIZE = 256
ENCODER_INPUT_SIZE = 3*ENCODER_SAMPLE_SIZE
ENCODER_HIDDEN_SIZE = [128, 64, 32]
...
params_dict = {
    'ENCODER_SAMPLE_SIZE': str(ENCODER_SAMPLE_SIZE),
    'ENCODER_INPUT_SIZE': str(ENCODER_INPUT_SIZE),
    'ENCODER_HIDDEN_SIZE': str(ENCODER_HIDDEN_SIZE),
    'ENCODER_OUTPUT_SIZE': str(ENCODER_OUTPUT_SIZE),
...
def train(args):
    ...
    if GRID64 or GRID128:
        ...
        vessel_dataset = VesselDatasetSinglePoint(DATA_PATH, sample_size=ENCODER_SAMPLE_SIZE, transform_function=transform_function)
    ...
    encoder = MLP([ENCODER_INPUT_SIZE] + ENCODER_HIDDEN_SIZE + [ENCODER_OUTPUT_SIZE])
    mlp = MLP([INPUT_SIZE+ENCODER_OUTPUT_SIZE] + HIDDEN_SIZE + [OUTPUT_SIZE])
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        model.train()
        epoch_mse_list = []
        for i, (input_tensor, target_tensor, geometry_tensor) in enumerate(train_loader):
            input_tensor = input_tensor.to(DEVICE)
            target_tensor = target_tensor.to(DEVICE)
            geometry_tensor = geometry_tensor.to(DEVICE)

            pred_tensor = model(input_tensor, geometry_tensor)
        ...
        with torch.no_grad():  # Disable gradient computation
            for i, (input_tensor, target_tensor, geometry_tensor) in enumerate(val_loader):
                input_tensor = input_tensor.to(DEVICE)
                target_tensor = target_tensor.to(DEVICE)
                geometry_tensor = geometry_tensor.to(DEVICE)
